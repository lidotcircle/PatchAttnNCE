...
def generate_images(
    ...
    for i, imgs in tqdm(enumerate(dataloader)):
        ...
        styles = G.styleformer(img)
        styles_ex = torch.zeros_like(styles)
        styles_ex[0] = styles[1]
        styles_ex[1] = styles[0]
        out_img = G(img, in_styles=styles)
        out_img2 = G(img, in_styles=styles_ex)
