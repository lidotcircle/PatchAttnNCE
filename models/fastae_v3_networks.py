...
class ResnetBlock(nn.Module):
    def __init__(self, dim: int, norm: str):
        super().__init__()
        conv_block = [
            conv2d(dim, dim*2, kernel_size=3, padding=1, bias=False),
            NormLayer(dim*2, norm),
            GLU(),
            conv2d(dim, dim, kernel_size=3, padding=1, bias=False),
        ]
...
class ResnetBlockStyle(nn.Module):
    def __init__(self, dim: int, style_dim: int):
        super().__init__()
        self.sty2params = nn.Sequential(
            linear(style_dim, style_dim * 2),
            nn.ReLU(),
            linear(style_dim * 2, dim * 4)
        )

        self.conv1 = conv2d(dim, dim*2, kernel_size=3, padding=1, bias=False)
        self.adaIN = AdaIN(dim*2)
        self.glu = GLU()
        self.conv2 = conv2d(dim, dim, kernel_size=3, padding=1, bias=False)
...
class ContentEncoder(nn.Module):
    def __init__(self, ngf=128, img_resolution=256, nc=3):
        ...
        for _ in range(3):
            layers += [ ResnetBlock(nfc[32], 'instance') ]
...
class StyleEncoder(nn.Module):
    def __init__(self, latent_dim, lite: bool=False, ngf=128, img_resolution=256, nc=3):
        super().__init__()
        self.img_resolution = img_resolution
        nfc_multi = {2: 16, 4:16, 8:8, 16:4, 32:2, 64:2, 128:1, 256:0.5,
                     512:0.25, 1024:0.125}
        ...
        if not lite:
            layers += [
                nn.Flatten(),
                EqualLinear(nfc[4] * 4 * 4, nfc[4], activation='fused_lrelu'), 
                EqualLinear(nfc[4], latent_dim),
            ]
        else:
            layers += [
                nn.AdaptiveAvgPool2d(1),
                nn.Flatten(),
                EqualLinear(nfc[4], nfc[4], activation='fused_lrelu'), 
                EqualLinear(nfc[4], latent_dim),
            ]
        self.layers = nn.Sequential(*layers)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim: int, style_dim: int=512, ngf: int=128, nc=3, img_resolution=256, lite: bool=False):
        ...
        self.res1 = ResnetBlockStyle(nfc[32], style_dim)
        self.res2 = ResnetBlockStyle(nfc[32], style_dim)
        self.res3 = ResnetBlockStyle(nfc[32], style_dim)
        ...
        self.se_64  = SEBlock(nfc[32], nfc[64])
    ...
    def forward(self, content, z):
        ...
        res1_32 = self.res1(content, style)
        res2_32 = self.res2(res1_32, style)
        res3_32 = self.res3(res2_32, style)
        feat_64 = self.se_64(content, self.sfeat_64(res3_32, style))
        feat_128 = self.se_128(res1_32, self.sfeat_128(feat_64, style))
        feat_256 = self.se_256(res2_32, self.sfeat_256(feat_128, style))
        feat_last = feat_256
