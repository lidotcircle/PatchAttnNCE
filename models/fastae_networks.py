...
class ContentEncoder(nn.Module):
    def __init__(self, ngf=128, img_resolution=256, nc=3):
        ...
        layers += [ DownBlock(nfc[16], nfc[8], 'instance') ]
...
class Decoder(nn.Module):
    ...
    def forward(self, content, style):
        feat_16 = self.sfeat_16(content, style)
        feat_32 = self.sfeat_32(feat_16, style)
        feat_64 = self.se_64(content, self.sfeat_64(feat_32, style))
        feat_128 = self.se_128(feat_16, self.sfeat_128(feat_64, style))
        feat_256 = self.se_256(feat_32, self.sfeat_256(feat_128, style))
        feat_last = feat_256
