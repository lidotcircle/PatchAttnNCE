...
class Generator(nn.Module):
    ...
    def forward(self, img, layers: list=None, z=None, encode_only: bool=False):
        ...
        content = feat
        feat_32 = decoder.sfeat_32(feat, style)
        layer_id = len(self.encoder.content_encoder.layers)
        if layer_id in layers:
            feats.append(feat_32)
        if layer_id >= last_layer:
            return feats

        feat_64 = decoder.se_64(content, decoder.sfeat_64(feat_32, style))
        layer_id = layer_id + 1
        if layer_id in layers:
        ...
        feat_128 = decoder.se_128(feat_32, decoder.sfeat_128(feat_64, style))
        layer_id = layer_id + 1
        if layer_id in layers:
        ...
        feat_256 = decoder.sfeat_256(feat_128, style)
        layer_id = layer_id + 1
        if layer_id in layers:
